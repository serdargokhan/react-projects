...
function Exchanges() {
    ...
    useEffect(() => {
        async function fetchExchanges() {
            const response = await fetch(`/v1/exchange/map?CMC_PRO_API_KEY=${API_KEY}&limit=20&sort=volume_24h`);
    ...
    useEffect(() => {
        async function getExchangeInformations() {
            const response = await fetch(`/v1/exchange/info?CMC_PRO_API_KEY=dca22541-b717-4053-8bc5-608d8e173179&id=${idArray.join()}`);
            const data = await response.json();
